...
export default function ContractorOrderDetail({order,onAdvancePrinter,onAdvanceRobot,onCompleteManual,onAcceptChange}:{order:Order;onAdvancePrinter:()=>void;onAdvanceRobot:()=>void;onCompleteManual:()=>void;onAcceptChange:()=>void}){
  return <section className="contractor-detail"><div className="order-detail-head"><div><b>{order.id}</b><span>{order.customer} · {order.projectName}</span></div><div className="detail-status">Design V{order.approvedVersion}</div></div>
    {order.changeRequest&&order.changeRequest.status==='submitted'&&<div className="change-alert"><div><strong>收到设计变更</strong><span>V{order.changeRequest.fromVersion} → V{order.changeRequest.toVersion} · 该 Draft 已保存住户最新方案与家具位置。接受后会重新生成 BOM 和后续任务。</span></div><button onClick={onAcceptChange}>接受并同步后续</button></div>}
    {order.changeRequest&&order.changeRequest.status==='accepted'&&<div className="downstream-sync-alert"><strong>Design V{order.approvedVersion} 已同步</strong><span>BOM、打印任务和施工任务已经按最新批准设计重新生成。</span></div>}
    <div className="contractor-board">
      <section className="production-side"><div className="board-title">生产</div><div className="bom-section"><h3>BOM清单及排序展示 <small>· Design V{order.downstreamVersion??order.approvedVersion}</small></h3><div className="bom-head"><span>顺序</span><span>构件</span><span>数量</span><span>来源</span><span>状态</span></div>{order.bom.map(x=><div className="bom-line" key={x.id}><span>{String(x.order).padStart(2,'0')}</span><strong>{x.label}</strong><span>{x.quantity}</span><span>{x.source}</span><span>{x.status}</span></div>)}</div>
