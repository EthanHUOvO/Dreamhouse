...
export default function ContractorPortal(){
  ...
  function advancePrinter(){mutate(o=>{const p=Math.min(100,o.printer.progress+12),done=p>=100;return {...o,printer:{...o.printer,progress:p,status:done?'完成':'运行中'},productionProgress:Math.max(o.productionProgress,p),bom:o.bom.map(x=>x.category==='furniture'?x:{...x,status:done?'已完成':'生产中'}),status:done&&o.status==='production'?'transport':o.status}})}
  function advanceRobot(){mutate(o=>{const p=Math.min(100,o.robot.progress+12),done=p>=100;return {...o,robot:{...o.robot,progress:p,status:done?'完成':'运行中'},constructionProgress:Math.max(o.constructionProgress,p),robotTasks:o.robotTasks.map((x,i)=>({...x,status:done?'已完成':i===0?'已完成':'施工中'})),status:done?'acceptance':o.status}})}
