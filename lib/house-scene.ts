...
const ASSETS = {
  ...
  chair: { id:'livingroom-chair', category:'furniture', name:'Chair', thumbnail:'/items/livingroom-chair/thumbnail.webp', src:'/items/livingroom-chair/model.glb', dimensions:[1.5,0.8,1.5], offset:[0.01,0,0], rotation:[0,0,0], scale:[1,1,1] },
...
function wall(id:string, start:[number,number], end:[number,number], structural:'load_bearing'|'partition', children:string[]=[]): SceneNode {
  return {
    object:'node', id, type:'wall', parentId:LEVEL, visible:true, name:structural==='load_bearing'?'承重墙':'非承重墙',
    children, thickness:structural==='load_bearing'?0.22:0.12, height:H, start, end,
    frontSide:'unknown', backSide:'unknown',
    metadata:{structural_type:structural, editable:structural==='partition', lifecycle_scope:structural==='load_bearing'?'base':'scenario'}
  }
}

function door(id:string, wallId:string, localX:number, width=.86, options:Partial<SceneNode>={}): SceneNode {
  return {
    object:'node', id, type:'door', parentId:wallId, wallId, visible:true, name:'门',
    position:[localX,1.05,0], rotation:[0,0,0], width, height:2.1,
    constructionType:'framed', dimensionReference:'nominal', doorCategory:'interior', doorType:'hinged', leafCount:1,
    operationState:0, slideDirection:'left', trackStyle:'none', garagePanelCount:4,
    openingKind:'door', openingShape:'rectangle', openingRadiusMode:'all', openingTopRadii:[0.15,0.15], cornerRadius:0.08, archHeight:0.45, openingRevealRadius:0.025,
    ...
    handle:true, handleHeight:1.05, handleSide:'right', contentPadding:[0.04,0.04], doorCloser:false, panicBar:false, panicBarHeight:1,
    metadata:{lifecycle_scope:'base', opening_role:'door'}, ...options
  }
}

function windowNode(id:string, wallId:string, localX:number, width:number, height=1.45, sillHeight=1.0, options:Partial<SceneNode>={}): SceneNode {
  return {
    object:'node', id, type:'window', parentId:wallId, wallId, visible:true, name:'窗',
    position:[localX,sillHeight+height/2,0], rotation:[0,0,0], width, height,
    constructionType:'framed', dimensionReference:'nominal', openingKind:'window', windowType:'sliding', operationState:0,
    awningDirection:'up', casementStyle:'single', hingesSide:'left', openingShape:'rectangle', openingRadiusMode:'all', openingCornerRadii:[0.04,0.04,0.04,0.04], cornerRadius:0.04, archHeight:0.35, openingRevealRadius:0.025,
    frameThickness:0.055, frameDepth:0.07, columnRatios:[0.5,0.5], rowRatios:[1], columnDividerThickness:0.035, rowDividerThickness:0.03,
    sill:true, sillDepth:0.1, sillThickness:0.035,
    metadata:{lifecycle_scope:'base', opening_role:'window'}, ...options
  }
}
...
function zone(id:string,name:string,semantic:string,polygon:[number,number][],color:string): SceneNode {
  return {
    ...
    autoFromWalls:false, boundaryWallIds:[], spaceRole:'room', roomNumber:'', enclosureStatus:'auto', floorFinish:'wood', wallFinish:'paint', ceilingFinish:'paint', ceilingHeight:2.7, occupancy:'residential', clearDimensionPolicy:'none', color,
...
export function furnitureItem(id:string, roomId:string, assetKey:keyof typeof ASSETS, position:[number,number,number], rotationY=0, scale:[number,number,number]=[1,1,1]): SceneNode {
  ...
  return {
    ...
    asset:{...asset},
    metadata:{room_id:roomId,lifecycle_scope:'scenario',furniture_role:asset.id}
  }
}
...
export function createInitialHouseScene(): SceneGraph {
  ...
  nodes.site_house={object:'node',id:'site_house',type:'site',parentId:null,visible:true,name:'Ideal Home Site',polygon:{type:'polygon',points:[[-10,-8],[10,-8],[10,8],[-10,8]]},children:['building_house'],metadata:{house_id:'HOUSE_001'}}
  nodes.building_house={object:'node',id:'building_house',type:'building',parentId:'site_house',visible:true,name:'HOUSE_001',children:[LEVEL],position:[0,0,0],rotation:[0,0,0],metadata:{immutable_footprint:true}}

  const openings:SceneNode[]=[
    door('door_entry','wall_e',0.6,1.05,{name:'入户门',doorType:'hinged',leafCount:1,metadata:{lifecycle_scope:'base',opening_role:'entry'}}),
    door('door_living','wall_n_band',-2.2,1.05,{name:'客餐厅门',openingKind:'opening',handle:false,threshold:false}),
    door('door_study','wall_n_band',3.6,.86,{name:'书房门'}),
    door('door_master','wall_s_band',-3.8,.9,{name:'主卧门'}),
    door('door_bath','wall_s_band',0.45,.82,{name:'卫生间门'}),
    door('door_gaming','wall_s_band',3.6,.9,{name:'电竞房门'}),

    windowNode('win_n_living_1','wall_n',-4.1,1.65,1.45,.9,{name:'客厅北窗1'}),
    windowNode('win_n_living_2','wall_n',-1.3,1.65,1.45,.9,{name:'客厅北窗2'}),
    windowNode('win_n_study','wall_n',3.8,1.75,1.45,.9,{name:'书房北窗'}),
    windowNode('win_s_master','wall_s',-3.9,1.8,1.45,.9,{name:'主卧南窗'}),
    windowNode('win_s_bath','wall_s',0.0,.75,.75,1.65,{name:'卫生间高窗',windowType:'fixed',columnRatios:[1]}),
    windowNode('win_s_gaming','wall_s',3.8,1.8,1.45,.9,{name:'电竞房南窗'}),
    windowNode('win_w_living','wall_w',-2.3,1.4,1.35,.95,{name:'客厅西窗'}),
    windowNode('win_w_master','wall_w',2.7,1.4,1.35,.95,{name:'主卧西窗'}),
    windowNode('win_e_study','wall_e',-2.5,1.4,1.35,.95,{name:'书房东窗'}),
    windowNode('win_e_gaming','wall_e',2.8,1.4,1.35,.95,{name:'电竞房东窗'})
  ]
  ...
  nodes[FLOOR]={object:'node',id:FLOOR,type:'slab',parentId:LEVEL,visible:true,name:'住宅地板',polygon:[[-6,-4.5],[6,-4.5],[6,4.5],[-6,4.5]],holes:[],holeMetadata:[],elevation:.05,thickness:.08,recessed:false,autoFromWalls:false,metadata:{lifecycle_scope:'base',manufacturing_layer:'floor'}}
  ...
  const items=[
    furnitureItem('item_master_bed','zone_master','doubleBed',[-4.2,0,3.0],0),
    furnitureItem('item_master_bedside','zone_master','bedside',[-5.35,0,3.6],0),
  ...
  nodes[LEVEL]={
    object:'node',id:LEVEL,type:'level',parentId:'building_house',visible:true,name:'住宅一层',level:0,baseElevation:0,height:H,
