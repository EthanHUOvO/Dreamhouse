...
function wall(
  ...
  return {
    ...
    metadata:{
      ...
      editable:structural==='partition',
      lifecycle_scope:structural==='load_bearing'?'base':'scenario',
    },
  }
}
...
/**
 * IMPORTANT:
 * Pascal wall child position[0] is DISTANCE IN METERS FROM wall.start,
 * not a world X/Z coordinate and not an offset from wall center.
 */
function door(
  id:string,
  wallId:string,
  ...
  return {
    ...
    panicBar:false,
    panicBarHeight:1,
    metadata:{lifecycle_scope:'base',opening_role:'door'},
    ...options,
  }
}
...
function windowNode(
  ...
  return {
    ...
    visible:true,
    name:'窗',
    position:[distanceFromWallStart,sillHeight+height/2,0],
    rotation:[0,0,0],
    width,
    height,
    constructionType:'framed',
    ...
    openingShape:'rectangle',
    openingRadiusMode:'all',
    openingCornerRadii:[0.04,0.04,0.04,0.04],
    cornerRadius:0.04,
    archHeight:0.35,
    openingRevealRadius:0.025,
    ...
    sillDepth:0.1,
    sillThickness:0.035,
    metadata:{lifecycle_scope:'base',opening_role:'window'},
    ...options,
  }
}
...
export function createInitialHouseScene(): SceneGraph {
  ...
  nodes.site_house={
    ...
    parentId:null,
    visible:true,
    name:'Ideal Home Site',
    // Keep site close to the building so any auto-framing also favors the house.
  ...
  /*
   * Correct wall-local distances.
   *
   * wall_n / wall_s: start x=-6, length 12 => distance = desiredWorldX + 6
   * wall_w / wall_e: start z=-4.5, length 9 => distance = desiredWorldZ + 4.5
   * wall_n_band / wall_s_band: start x=-6, length 12 => distance = desiredWorldX + 6
   */
  const openings:SceneNode[]=[
    // East wall: desired z≈0.6 => 0.6 + 4.5 = 5.1m from wall start.
    door('door_entry','wall_e',5.10,1.05,{
      name:'入户门',
      doorType:'hinged',
      leafCount:1,
      metadata:{lifecycle_scope:'base',opening_role:'entry'},
    }),

    // Internal horizontal walls.
    door('door_living','wall_n_band',3.80,1.05,{
      name:'客餐厅门',
      openingKind:'opening',
      handle:false,
      threshold:false,
    ...
    windowNode('win_s_bath','wall_s',6.00,.75,.75,1.65,{
      name:'卫生间高窗',
      windowType:'fixed',
      columnRatios:[1],
    }),
  ...
  const walls:SceneNode[]=[
    wall('wall_n',[-6,-4.5],[6,-4.5],'load_bearing',[
      'win_n_living_1','win_n_living_2','win_n_study',
    ]),
    wall('wall_e',[6,-4.5],[6,4.5],'load_bearing',[
      'win_e_study','door_entry','win_e_gaming',
    ]),
    wall('wall_s',[-6,4.5],[6,4.5],'load_bearing',[
      'win_s_master','win_s_bath','win_s_gaming',
    ]),
    wall('wall_w',[-6,-4.5],[-6,4.5],'load_bearing',[
      'win_w_living','win_w_master',
    ]),
  ...
  nodes[FLOOR]={
    ...
    visible:true,
    name:'住宅地板',
    polygon:[[-6,-4.5],[6,-4.5],[6,4.5],[-6,4.5]],
    holes:[],
    holeMetadata:[],
    elevation:.05,
    thickness:.08,
    recessed:false,
    autoFromWalls:false,
    metadata:{lifecycle_scope:'base',manufacturing_layer:'floor'},
  }
